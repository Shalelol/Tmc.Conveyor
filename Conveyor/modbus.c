...
static int read_reg_response(modbus_param_t *mb_param,
                             uint16_t *data_dest, uint8_t *query, int max_data_dest);
...
#ifdef NATIVE_WIN32
			{
				ret = send( mb_param->sock, query, query_length, 0 );
				/*if (ret == SOCKET_ERROR) {
					ret = -1;
				}*/
			}				
                          ...
                          int start_addr, int nb, uint16_t *data_dest)
{
        ...
        query_ret = modbus_send(mb_param, query, query_length);
        if (query_ret > 0)
                status = read_reg_response(mb_param, data_dest, query, nb);
        else
                status = query_ret;
...
static int read_reg_response(modbus_param_t *mb_param, uint16_t *data_dest,
                             uint8_t *query, int max_data_dest)
{
        uint8_t response[MAX_MESSAGE_LENGTH];
        int response_ret;
        int offset;
        ...
        if ((response_ret >=0) && (max_data_dest > 0) && (response_ret != max_data_dest)) {
        	/* Someway more registers than required returned ? Discard everything. */
        	response_ret = -1;
        }
...
#ifdef UNICODE
		MultiByteToWideChar(CP_ACP,0,mb_param->device,-1,device,16);
#else
		strncpy(device,mb_param->device,16);
#endif

		/* mb_param->device should contain a string like "COMxx:" xx being a decimal number */
		mb_param->w_ser.fd = CreateFile(device,
...
static int modbus_connect_tcp(modbus_param_t *mb_param)
{
        ...
        mb_param->sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        if (mb_param->sock == INVALID_SOCKET) {
        ...
        // SOL_TCP = IPPROTO_TCP 
        option = 1;
        ret = setsockopt(mb_param->sock, IPPROTO_TCP, TCP_NODELAY,
                         (const void *)&option, sizeof(int));
        if (ret != 0) {
                closesocket(mb_param->sock);
				WSACleanup();
				mb_param->sock = INVALID_SOCKET;
                return ret;
        }

        // Set the IP low delay option
		/* IP_TOS not compatible with winXP (only win2000)
        option = IPTOS_LOWDELAY;
        ret = setsockopt(mb_param->sock, IPPROTO_TCP, IP_TOS,
                         (const void *)&option, sizeof(int));
        if (ret != 0) {
                perror("setsockopt");
                close(mb_param->sock);
		...
		*/
        ...
        // Connect to server.
        iResult = connect( mb_param->sock, (SOCKADDR *)&addr, sizeof(addr));
...
{		
		...
		iResult = bind(new_socket, (SOCKADDR *)&addr, sizeof(SOCKADDR_IN));
		if (iResult == SOCKET_ERROR) {
		...
		mb_param->sock = accept(new_socket, NULL, NULL);
		if (mb_param->sock == INVALID_SOCKET) {
			closesocket(new_socket);
			WSACleanup();
			exit(1);
